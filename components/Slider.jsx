    ...
    const fetchMovies = async () => {
      try {
        const response = await fetch(
          "https://api.themoviedb.org/3/trending/movie/day?api_key=97c1ec10f492d5880ccb5f65506d37e0&page=1"
        );
        if (!response.ok) {
          throw new Error("Failed to fetch movies");
        }
        const data = await response.json();
        setMovies(data.results.slice(0, 6)); // Limit to 6 movies
      } catch (error) {
      ...
      <div className="relative w-full h-full carousel">
        {movies.map((movie, index) => (
          <div
            key={movie.id}
            id={`slide${index + 1}`}
            className={`relative w-full h-full carousel-item ${index + 1 === currentSlide ? "" : "hidden"
              }`}
          >
            ...
            <div className="absolute z-20 bg-transparent card w-[40%] xl:bottom-20 left-20 bottom-0">
              <div className="card-body">
                <h2 className="text-3xl text-slate-300 card-title">{movie.title}</h2>
                <p className="font-medium text-slate-300 line-clamp-3">{movie.overview}</p>
